...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        # Set up action encoder if latent_dim > 0
        self.latent_ac_dim = latent_ac_dim
        if self.latent_ac_dim > 0:
            assert ac_encoder is not None and ac_decoder is not None 
            self.ac_encoder = ac_encoder
            self.ac_decoder = ac_decoder
    ...
    def _setup_model(self) -> None:
        ...
        if self.latent_ac_dim > 0:
            self.latent_action_space = gym.spaces.Box(
                low=-1, 
                high=1, 
                shape=(self.latent_ac_dim,), 
                dtype=np.float32
            )
            self.policy_action_space = self.latent_action_space
        else:
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            if self.latent_ac_dim > 0:
                unscaled_lat_ac, _ = self.predict(self._last_obs, deterministic=False)
                unscaled_action = self.ac_decoder(self._last_obs, unscaled_lat_ac)
            else:
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            if self.latent_ac_dim > 0:
                # Assume latent action prediction is already in [-1, 1]
