...
class BaseAlgorithm(ABC):
    ...
    def __init__(
        ...
        if env is not None:
            ...
            # self._act_dim = self.action_space.shape[0]
    ...
    def _setup_learn(
        ...
        if reset_num_timesteps or self._last_obs is None:
            ...
            # if self._lat_dim > 0:
            #     self._last_lats = np.zeros((self.env.num_envs, self._lat_dim*2), dtype=np.float32)
            #     self._last_acts = np.zeros((self.env.num_envs, self._act_dim), dtype=np.float32)
