...
class SAC(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        latent_ac_dim: Optional[int] = -1,
        ac_encoder: Callable[[th.Tensor, th.Tensor], th.Tensor] = None,
        ac_decoder: Callable[[th.Tensor, th.Tensor], th.Tensor] = None,
    ):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            observations, actions = replay_data.observations, replay_data.actions
            next_observations = replay_data.next_observations
            rewards, dones = replay_data.rewards, replay_data.dones

            # Encode actions if latent is provided
            if self.latent_ac_dim > 0:
                with th.no_grad():
                    actions = self.ac_encoder(replay_data.observations, actions)

            # Compute learned reward if reward function is provided
            true_rewards.append(rewards.mean().item())
            if self.reward is not None:
                with th.no_grad():
                    rewards = self.reward(observations, actions)
                    learned_rewards.append(rewards.mean().item())
        ...
        self.logger.record("train/q_pi", np.mean(min_qf_pis))
        self.logger.record("train/true_rewards", np.mean(true_rewards))
        if len(learned_rewards) > 0:
            self.logger.record("train/learned_rewards", np.mean(learned_rewards))
